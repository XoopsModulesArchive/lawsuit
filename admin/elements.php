...
'upload' => _AM_ELE_UPLOADFILE,
    ...
    echo '<div align="center">
			<form action="' . LAWSUIT_URL . 'admin/editelement.php" method="post">
				<b>' . _AM_ELE_CREATE . '</b>';
    ...
    if ($elements = &$lawsuit_ele_mgr->getObjects($criteria)) {
        $cell = 'even';

        foreach ($elements as $i) {
            $cell = 'even' == $cell ? 'odd' : 'even';
    ...
    foreach ($ele_id as $id) {
        $element = $lawsuit_ele_mgr->get($id);
    ...
    if (empty($error)) {
        if (_AM_SAVE_THEN_FORM == $_POST['submit']) {
            redirect_header(LAWSUIT_ADMIN_URL . '?op=edit&amp;form_id=' . $form_id, 0, _AM_DBUPDATED);
        } else {
