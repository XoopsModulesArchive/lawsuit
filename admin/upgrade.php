...
// $Id: upgrade.php,v 1.05 2009/06/24 23:45:00 wishcraft Exp $

require __DIR__ . '/admin_header.php';
$version = number_format($xoopsModule->getVar('version') / 100, 2);
$count   = $lawsuit_form_mgr->getCount(new Criteria(1));
if ($version >= 1.2 || $count > 0) {
    ...
    $sql[] = 'ALTER TABLE `' . $xoopsDB->prefix('liaise') . '` RENAME `' . $xoopsDB->prefix('lawsuit_formelements') . '`';

    $msgs[] = 'Add form_id to elements table...';

    $sql[] = 'ALTER TABLE `' . $xoopsDB->prefix('lawsuit_formelements') . "` ADD `form_id` SMALLINT( 5 ) DEFAULT '1' NOT NULL AFTER `ele_id`";

    $msgs[] = 'Change default value of form_id in elements table...';

    $sql[] = 'ALTER TABLE `' . $xoopsDB->prefix('lawsuit_formelements') . "` CHANGE `form_id` `form_id` SMALLINT( 5 ) DEFAULT '0' NOT NULL";
    ...
    $sql[] = 'CREATE TABLE `' . $xoopsDB->prefix('lawsuit_forms') . "` (
    ...
    $sql[] = 'INSERT INTO `' . $xoopsDB->prefix('lawsuit_forms') . "` VALUES (1, '" . $method . "', " . (int)$sendto . ", 1, '" . $delimiter . "', 'Contact Us', '" . _SUBMIT . "', 'Tell us about your comments for this site.', 'Contact us by filling out this form.', '');";
    ...
    if (false === $error) {
        ...
        for ($i = 1; $i < 4; $i++) {
            ...
            $perm->setVar('gperm_name', $lawsuit_form_mgr->perm_name);
    ...
    xoops_confirm(['goupgrade' => 1], LAWSUIT_URL . 'admin/upgrade.php', 'Make sure you have your files and database backuped. Are you really ready to upgrade the module now?', 'Cut the crap and do it');
}
