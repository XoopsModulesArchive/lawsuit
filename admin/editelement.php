...
// $Id: editelement.php,v 1.05 2009/06/24 23:45:00 wishcraft Exp $

include 'admin_header.php';
$lawsuit_ele_mgr = xoops_getModuleHandler('elements');
require_once LAWSUIT_ROOT_PATH . 'class/elementrenderer.php';
define('_THIS_PAGE', LAWSUIT_URL . 'admin/editelement.php');
$myts  = MyTextSanitizer::getInstance();
$count = $lawsuit_form_mgr->getCount();
if ($count < 1) {
    redirect_header(LAWSUIT_ADMIN_URL, 0, _AM_GO_CREATE_FORM);
}
...
switch ($op) {
    case 'edit':
        ...
        switch ($ele_type) {
            ...
            case 'html':
                ...
                require __DIR__ . '/ele_html.php';
                break;
            case 'uploadimg':
                require __DIR__ . '/ele_uploadimg.php';
                break;
            case 'upload':
                require __DIR__ . '/ele_upload.php';
                break;
        }
        ...
        if (true === $clone || empty($form_id)) {
            ...
            $forms = &$lawsuit_form_mgr->getObjects(null, 'form_id, form_title');
    ...
    case 'delete':
        ...
        if (empty($_POST['ok'])) {
            ...
            $lawsuit_ele_mgr->delete($element);

            redirect_header(LAWSUIT_URL . 'admin/elements.php?form_id=' . $form_id, 0, _AM_DBUPDATED);
        }
        break;
    case 'save':
        if (!empty($ele_id)) {
        ...
        if (!$lawsuit_ele_mgr->insert($element)) {
            xoops_cp_header();
